...
if(rs.next()){
	...
	if(rs.getString("pass").equals(pass)){
		...
		sql = "update member set name = ? , sex = ?, age = ?, email = ?  where  id = ? and pass = ?"; //Statement 형태
		pstmt = con.prepareStatement(sql);

		pstmt.setString(1, name); //첫번째 물음표 값
		pstmt.setString(2, sex); //두번째물음표, 값
		pstmt.setString(3, age); //3번째물음표, 값
		pstmt.setString(4, email); //4번째물음표, 값
		pstmt.setString(5, id); //5번째물음표, 값
		pstmt.setString(6, pass); //6번째물음표, 값
